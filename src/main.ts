...
app.whenReady().then(() => {
  ...
  ipcMain.handle('foundry-local-app:update-chat-session-model', (_event, sessionId: string, modelId: string) => foundryAppService.updateChatSessionModel(sessionId, modelId));
  ...
  ipcMain.handle('foundry-local-app:stop-chat-response', (_event, sessionId: string) => foundryAppService.stopChatResponse(sessionId));
...
app.on('will-quit', () => {
  ...
  ipcMain.removeHandler('foundry-local-app:update-chat-session-model');
  ...
  ipcMain.removeHandler('foundry-local-app:stop-chat-response');
